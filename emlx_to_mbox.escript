...
main([Dir, "-o", Mbox]) ->
    Messages = filelib:fold_files(Dir, ".*\.emlx$", true, fun(File, Acc) -> [parse_emlx_filename(File) | Acc] end, []),
...
parse_emlx_filename(File) ->
    ...
    MessageID = case filename:basename(Basename, ".partial") of
        Basename -> list_to_integer(Basename);
        RealBasename -> list_to_integer(RealBasename)
    end,
    {MessageID, File}.

process_emlx_file(MessageID, File, Output, {AccDupes, AccMissingRemoteID, AccMissingAttachments, AccTree}) ->
    Basename = filename:basename(File, ".emlx"),
    Partial = case filename:basename(Basename, ".partial") of
        Basename -> false;
        _RealBasename -> true
    end,
    ...
    <<"<plist version=\"1.0\">">>, <<"<dict>">> | XMLLines1] = XMLLines,
    {XMLDict, [<<"</plist>">>, <<>>]} = parse_dict(XMLLines1, []),
    file:close(Fd),
    {<<"flags">>, {integer, Flags}} = lists:keyfind(<<"flags">>, 1, XMLDict),
    RemoteID = case lists:keyfind(<<"remote-id">>, 1, XMLDict) of
    ...
    ContentLengthHeader = list_to_binary([<<"Content-Length: ">>, integer_to_list(ContentLength), <<"\n">>]),
    ReturnPath = case binary:split(Headers, <<"\n">>) of
    ...
    DateReceived = case lists:keyfind(<<"date-received">>, 1, XMLDict) of
        {<<"date-received">>, {integer, DateReceivedInt}} ->
            calendar:gregorian_seconds_to_datetime(calendar:datetime_to_gregorian_seconds({{1970,1,1},{0,0,0}}) + DateReceivedInt);
        false -> filelib:last_modified(File)
    end,
    ...
    if
        ...
        MissingAttachment ->
            case gb_trees:is_defined(RemoteID, AccTree) of
                true -> {AccDupes + 1, AccMissingAttachments, AccTree};
                false ->
                    case gb_trees:is_defined(RemoteID, AccMissingAttachments) of
                        ...
                        false ->
                            {AccDupes, AccMissingRemoteID, gb_trees:insert(RemoteID, FinalContent, AccMissingAttachments), AccTree}
                    end
            end;
        true ->
            case gb_trees:lookup(RemoteID, AccTree) of
                {value, {RFC822MessageID, Partial, _PrevousMID, FlagHeaders, ContentLength}} -> {AccDupes + 1, AccMissingRemoteID, AccMissingAttachments, AccTree};
                {value, {PreviousRFC822MessageID, PreviousPartial, PreviousMID, PreviousFlagHeaders, PreviousContentLength}} ->
                    io:format("Warning: duplicate message ~s (~B/~B):\nmessage-id: ~s/~s\npartial: ~p/~p\nflags : ~s\nprevious flags: ~s\nlength : ~B/~B\n", [RemoteID, MessageID, PreviousMID, RFC822MessageID, PreviousRFC822MessageID, Partial, PreviousPartial, FlagHeaders, PreviousFlagHeaders, ContentLength, PreviousContentLength]),
                ...
                none ->
                    ...
                    {AccDupes,
                     ...
                     gb_trees:insert(RemoteID, {RFC822MessageID, Partial, MessageID, FlagHeaders, ContentLength}, AccTree)}
           end
    end.
...
parse_dict([<<"</dict>">> | Rest], Acc) -> {lists:reverse(Acc), Rest}.
...
parse_value(<<"\t<real>", Tail/binary>>, Rest) ->
    ...
    Value = case binary:match(RealBin, <<".">>) of
        nomatch ->
...
parse_value(<<"\t<array>">>, Rest) ->
    {ArrayValues, [<<"\t</array>">> | Rest1]} = lists:splitwith(fun(ArrayValueLine) ->
        case ArrayValueLine of
            <<"\t</array>">> -> false;
            <<"\t\t", _/binary>> -> true
        end
    end, Rest),
    ArrayValuesL = lists:map(fun(<<"\t", ArrayValueLine/binary>>) ->
...
format_date({Date, Time}) ->
    ...
    MonthText = lists:nth(element(2, Date), ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"]),
    io_lib:format("~s ~s ~2.10.0B ~2.10.0B:~2.10.0B:~2.10.0B ~4.10.0B", [DOWText, MonthText, element(3, Date), element(1, Time), element(2, Time), element(3, Time), element(1, Date)]).

process_flags(Flags) ->
    FlagsBin = <<Flags:64>>,
    <<_Unknown:32, 0:1, _HighlightInToc:1, _JunkLevelRecorded:1, _FontSizeDelta:3, IsNotJunk:1, IsJunk:1, _Signed:1,
    _PriorityLevel:7,
    _AttachmentCount:6,
    _Redirected:1, Forwarded:1,
    _Initial:1, Draft:1, Recent:1, Flagged:1, _Encrypted:1, Answered:1, Deleted:1, Read:1>> = FlagsBin,
    StatusHeaderValue = list_to_binary([
        case Read of 1 -> <<"R">>; 0 -> <<>> end,
        case Recent of 1 -> <<"">>; 0 -> <<"O">> end
    ]),
    StatusHeader = case StatusHeaderValue of
        <<>> -> <<>>;
        StatusHeaderValueBin -> <<"Status: ", StatusHeaderValueBin/binary, "\n">>
    end,
    XStatusHeaderValue = list_to_binary([
        case Answered of 1 -> <<"A">>; 0 -> <<>> end,
        case Flagged of 1 -> <<"F">>; 0 -> <<>> end,
        case Draft of 1 -> <<"T">>; 0 -> <<>> end,
        case Deleted of 1 -> <<"D">>; 0 -> <<>> end
    ]),
    XStatusHeader = case XStatusHeaderValue of
        <<>> -> [];
        XStatusHeaderValueBin -> <<"X-Status: ", XStatusHeaderValueBin/binary, "\n">>
    end,
    XKeywordsHeaderValue = list_to_binary([
        case Forwarded of 1 -> <<" $Forwarded Forwarded">>; 0 -> <<>> end,
        case IsNotJunk of 1 -> <<" $NotJunk NotJunk">>; 0 -> <<>> end,
        case IsJunk of 1 -> <<" $Junk Junk">>; 0 -> <<>> end
    ]),
    XKeywordsHeader = case XKeywordsHeaderValue of
        <<>> -> <<>>;
...
find_emlxpart(File) ->
    ...
    case Parts of
        [] -> [];
        _ ->
            Attachments = lists:map(fun(Part) ->
                ...
                AttachmentPartPath = [list_to_integer(Component) || Component <- re:split(PartNumberStr, "\\.", [{return, list}])],
...
find_opened_attachments(File) ->
    ...
    Attachments = lists:map(fun(AttachmentSubdir) ->
        [Attachment] = filelib:wildcard("*", filename:join(MessageAttachmentsDir, AttachmentSubdir)),
        AttachmentPath = filename:join([MessageAttachmentsDir, AttachmentSubdir, Attachment]),
        AttachmentPartPath = [list_to_integer(Component) || Component <- re:split(AttachmentSubdir, "\\.", [{return, list}])],
        {AttachmentPartPath, {file, AttachmentPath}}
    end, filelib:wildcard("*", MessageAttachmentsDir)),
    Attachments.
...
process_body(Headers, Body, File, true) ->
   ...
   MergedAttachments = lists:foldl(fun({PartPath, Attachment}, Acc) ->
       lists:keystore(PartPath, 1, Acc, {PartPath, Attachment})
   end, OpenedAttachments, EmlxPartAttachements),
...
process_body_with_attachments(Headers, Body, Attachments) ->
    ...
    case ContentType of
        <<"multipart/", _/binary>> ->
            ...
            {PartsWithAttachments, [], MissingAttachments} = insert_attachments_r(Parts, Attachments, [], false),
...
merge_multiparts(Boundary, Preamble, Epilogue0, Multiparts) ->
    Epilogue1 = case Epilogue0 of
        [] -> [];
        _ -> [<<"--">>, Boundary, <<"--\n">> | Epilogue0]
    end,
    list_to_binary([Preamble, [[<<"--">>, Boundary, <<"\n">>, Part] || Part <- Multiparts], Epilogue1]).
...
insert_attachment({AttachmentType, AttachmentFile}, Headers) ->
    ...
    HeaderLines = [HeaderLine || HeaderLine <- binary:split(Headers, <<"\n">>, [global]), HeaderLine =/= <<>>],
    {HeaderLinesFiltered, AppleContentLength, ContentTransferEncoding, _Filename} = parse_attachment_headers(HeaderLines, undefined, undefined, undefined, []),
    {Binary, Zipped} = case file:read_file(AttachmentFile) of
        {ok, Binary0} -> {Binary0, false};
        {error, eisdir} ->
            % Compress the directory
            _ = os:cmd("cd \"" ++ filename:dirname(AttachmentFile) ++ "\" && zip \"" ++ filename:basename(AttachmentFile) ++ ".zip\" \"" ++ filename:basename(AttachmentFile) ++ "\"/*"),
            {ok, Binary0} = file:read_file(AttachmentFile ++ ".zip"),
            file:delete(AttachmentFile ++ ".zip"),
            {Binary0, true}
    end,
    ContentTransferEncodingAtom = case ContentTransferEncoding of
        undefined -> undefined;
        _ -> list_to_atom(string:to_lower(binary_to_list(ContentTransferEncoding)))
    end,
    Encoded = case AttachmentType of
        file -> encode_file(ContentTransferEncodingAtom, Binary);
        part -> list_to_binary([Binary, <<"\n">>])
    end,
    if
        AppleContentLength =:= undefined -> ok;
        AppleContentLength =:= byte_size(Encoded) -> ok;
        AppleContentLength =:= byte_size(Encoded) + 1 andalso ContentTransferEncoding =:= <<"base64">> -> ok;
        AppleContentLength =:= byte_size(Encoded) - 1 andalso ContentTransferEncoding =:= <<"base64">> -> ok;
        Zipped ->
...
parse_attachment_headers([<<"X-Apple-Content-Length: ", LengthBin/binary>> | Tail], undefined, Encoding, Filename, AccFiltered) ->
    LengthInt = binary_to_integer(LengthBin),
    parse_attachment_headers(Tail, LengthInt, Encoding, Filename, AccFiltered);
parse_attachment_headers([<<"Content-Transfer-Encoding: ", TransferEncoding/binary>> = Header | Tail], Length, undefined, Filename, AccFiltered) ->
    parse_attachment_headers(Tail, Length, TransferEncoding, Filename, [Header | AccFiltered]);
parse_attachment_headers([<<"Content-Disposition: ", _/binary>> | _] = Headers, Length, TransferEncoding, undefined, AccFiltered) ->
...
parse_attachment_headers([Header | Tail], Length, TransferEncoding, Filename, AccFiltered) ->
    parse_attachment_headers(Tail, Length, TransferEncoding, Filename, [Header | AccFiltered]);
parse_attachment_headers([], Length, Encoding, Filename, FilteredR) -> {[[Header, <<"\n">>] || Header <- lists:reverse(FilteredR)], Length, Encoding, Filename}.

get_filename(Headers) ->
    {CDHeaders, Tail} = lists:splitwith(fun(Header) ->
        case Header of
...
get_filename0([H | Tail]) ->
    case binary:split(H, <<"filename">>) of
        ...
        [_, Rest] ->
            ...
            qp_decode(Filename);
        [H] -> get_filename0(Tail)
    end;
get_filename0([]) -> undefined.
...
percent_decode0([], Acc) -> lists:reverse(Acc).
...
qp_decode0([], Acc) -> lists:reverse(Acc).
...
quoted_printable(<<C, Next, Tail/binary>>, N, Acc) when C =:= 9 orelse C =:= 32 andalso Next =/= $\r andalso Next =/= $\n ->
...
quoted_printable(<<>>, _, Acc) -> list_to_binary(lists:reverse(Acc)).
...
wrap_lines0(Text, _Columns, Acc) -> list_to_binary(lists:reverse([[Text, <<"\n">>] | Acc])).

insert_attachments_r(Parts, Attachments, Path, Missing) ->
    {PartsWithAttachmentsR, RemainingAttachments, FinalMissing} = lists:foldl(fun({Ix, Part}, {AccParts, AccRemainingAttachments, AccMissing}) ->
        ...
        case lists:keyfind(CompletePath, 1, AccRemainingAttachments) of
            {CompletePath, AttachmentFile} ->
                ...
                {[RewrittenPart | AccParts], lists:keydelete(CompletePath, 1, AccRemainingAttachments), AccMissing};
            false ->
                ContentType = get_content_type(Headers),
                case ContentType of
                    <<"multipart/", _/binary>> ->
                        {Boundary, Preamble, Epilogue, SubParts} = split_multiparts(ContentType, Body),
                        {PartsWithAttachments, NewRemainingAttachments, NewMissing} = insert_attachments_r(SubParts, AccRemainingAttachments, CompletePath, AccMissing),
                        RewrittenBody = merge_multiparts(Boundary, Preamble, Epilogue, PartsWithAttachments),
                    ...
                    _ ->
                        case binary:match(Headers, <<"\nX-Apple-Content-Length:">>) of
                            nomatch -> {[Part | AccParts], AccRemainingAttachments, AccMissing};
                            _ ->
                                % Try harder to recover attachment.
                                HeaderLines = [HeaderLine || HeaderLine <- binary:split(Headers, <<"\n">>, [global]), HeaderLine =/= <<>>],
                                {_HeaderLinesFiltered, _AppleContentLength, _ContentTransferEncoding, Filename} = parse_attachment_headers(HeaderLines, undefined, undefined, undefined, []),
                                case lookup_open_attachment(Filename) of
                                    {value,  AttachmentFile} ->
                                        io:format("Warning: found ~s at ~s\n", [Filename, AttachmentFile]),
                                        {RewrittenHeaders, RewrittenBody} = insert_attachment({file, AttachmentFile}, Headers),
                                        RewrittenPart = list_to_binary([RewrittenHeaders, <<"\n">>, RewrittenBody]),
                                        {[RewrittenPart | AccParts], lists:keydelete(CompletePath, 1, AccRemainingAttachments), AccMissing};
                                    false ->
                                        if
    ...
    end, {[], Attachments, Missing}, lists:zip(lists:seq(1, length(Parts)), Parts)),
    {lists:reverse(PartsWithAttachmentsR), RemainingAttachments, FinalMissing}.

lookup_open_attachment(undefined) -> false;
lookup_open_attachment(File) ->
    HomeDir = os:getenv("HOME"),
    FileStr = unicode:characters_to_list(File),
    Result = os:cmd("mdfind -name \"" ++ FileStr ++ "\" | grep -E '^" ++ HomeDir ++ "/Library/Containers/com.apple.mail/Data/Library/Mail Downloads'"),
    case Result of
        [] -> false;
        _ ->
            Results = string:tokens(Result, "\n"),
            FilteredResults = lists:filter(fun(ThisResult) ->
